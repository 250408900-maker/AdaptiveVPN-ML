...
PCAP = Path("data/captures/normal/normal_001.pcapng")
OUTPUT = Path("data/processed/normal_001_features.csv")
...
def main():
    ...
    for line in result.stdout.splitlines():
        ...
        if rtt is not None:
            w["rtt_values"].append(rtt * 1000)  # seconds -> ms
    ...
    for window_id in sorted(windows):
        ...
        rows.append({
            ...
            "throughput_kbps": round(
                (w["total_bytes"] * 8) / WINDOW_SIZE / 1000, 2
            ),
            ...
            "retransmission_rate": round(
                w["retransmissions"] / max(w["tcp_packets"], 1), 5
            ),
            "tcp_resets": w["tcp_resets"],
            "reset_rate": round(
                w["tcp_resets"] / max(w["tcp_packets"], 1), 5
            ),
